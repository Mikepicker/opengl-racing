...
car* entities_new_car(vec3 pos, char* filename) {
  ...
  c->suspension_fl.x = (CAR_CG_TO_FRONT_AXLE_DISTANCE/CAR_FRAME_LONGITUDINAL_LENGTH)*CAR_MASS*GRAVITY_ACC*WEIGHT_TRANSFER_FACTOR/SUSPENSION_STIFFNESS;
  c->suspension_fl.v = 0;
  c->suspension_fr.x = (CAR_CG_TO_FRONT_AXLE_DISTANCE/CAR_FRAME_LONGITUDINAL_LENGTH)*CAR_MASS*GRAVITY_ACC*WEIGHT_TRANSFER_FACTOR/SUSPENSION_STIFFNESS;
  c->suspension_fr.v = 0;
  c->suspension_rl.x = (CAR_CG_TO_REAR_AXLE_DISTANCE/CAR_FRAME_LONGITUDINAL_LENGTH)*CAR_MASS*GRAVITY_ACC*WEIGHT_TRANSFER_FACTOR/SUSPENSION_STIFFNESS;
  c->suspension_rl.v = 0;
  c->suspension_rr.x = (CAR_CG_TO_REAR_AXLE_DISTANCE/CAR_FRAME_LONGITUDINAL_LENGTH)*CAR_MASS*GRAVITY_ACC*WEIGHT_TRANSFER_FACTOR/SUSPENSION_STIFFNESS;
  c->suspension_rr.v = 0;
...
// we use the front-wheel driving kinematic model
void car_update(car* car) {
    ...
    car->steering_wheel_angle += 25.0f * (car->steering_command - car->steering_wheel_angle) * microdrag.delta_time;
    ...
    car->pitch=1.0f*atanf((car->suspension_fl.x + car->suspension_fr.x - car->suspension_rl.x - car->suspension_rr.x)/2.0f/CAR_FRAME_LONGITUDINAL_LENGTH);
    car->roll=1.2f*atanf((car->suspension_fr.x + car->suspension_rr.x - car->suspension_fl.x - car->suspension_rl.x)/2.0f/CAR_FRAME_TRANSVERSAL_LENGTH);

    //height of car c.o.m.
    car->obj->position[1] = (car->suspension_fl.x + car->suspension_fr.x + car->suspension_rl.x + car->suspension_rr.x)/4.0;
}
...
void weight_transfer_on_suspensions(car *car) {
    ...
    // forward motion
    car->weight_transfer_front = WEIGHT_TRANSFER_FACTOR * (CAR_CG_TO_FRONT_AXLE_DISTANCE/CAR_FRAME_LONGITUDINAL_LENGTH*CAR_MASS*GRAVITY_ACC + CAR_CG_HEIGHT/CAR_FRAME_LONGITUDINAL_LENGTH*CAR_MASS*car->accel);
    car->weight_transfer_rear = WEIGHT_TRANSFER_FACTOR * (CAR_CG_TO_REAR_AXLE_DISTANCE/CAR_FRAME_LONGITUDINAL_LENGTH*CAR_MASS*GRAVITY_ACC - (CAR_CG_HEIGHT/CAR_FRAME_LONGITUDINAL_LENGTH)*CAR_MASS*car->accel);

    // rotational motion
    float lateral_acceleration = 3.0f*car->speed * ( tanf(car->steering_wheel_angle)/CAR_FRAME_LONGITUDINAL_LENGTH * 1000.0f*car->speed);
